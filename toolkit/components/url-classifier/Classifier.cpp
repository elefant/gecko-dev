...
void
Classifier::SwapInNewLookupCaches()
{
  // Assume we have
  //
  //   mLookupCaches:    [goog-phish-shavar, goog-unwanted-shavar]
  //   mNewLookupCaches: [goog-phish-shavar, goog-malware-shavar]
  //

  // Firstly, we swap new LookupCaches into mLookupCaches (according to the
  // table name) in a non-traditional way. See below for detail.
  for (auto &newOne: mNewLookupCaches) {
    for (auto &mayBeOld: mLookupCaches) {
      if (newOne->TableName() == mayBeOld->TableName()) {
        // Instead of deep swapping two LookupCaches, we swap their references
        // in the two arrays. Besides, the root directory handles have to be
        // swapped as well.
        Swap(newOne, mayBeOld);
        newOne->SwapRootDirHandle(mayBeOld);

        // newOne is pointing to the old LookupCache now. Delete it and null it out.
        delete newOne;
        newOne = nullptr;
      }
    }
  ...
  //
  //   mLookupCaches:    [goog-phish-shavar, goog-unwanted-shavar]
  //   mNewLookupCaches: [nullptr, goog-malware-shavar]
  //

  // Secondly, append newly created LookupCaches to mLookupCaches.
  for (auto newOne: mNewLookupCaches) {
    if (newOne) {
      mLookupCaches.AppendElement(newOne);
    }
  }
  ...
  // The final state is
  //
  //   mLookupCaches:    [goog-phish-shavar, goog-unwanted-shavar, goog-malware-shavar]
  //   mNewLookupCaches: []
  //
}

nsresult
Classifier::SwapInNewTables()
{
  nsresult rv;
