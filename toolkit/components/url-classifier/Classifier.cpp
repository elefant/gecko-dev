...
Classifier::Classifier(const ProviderDictType* aProviderDict)
{
  if (!aProviderDict) {
    return;
  }
  // Sadly that nsClassHashtable has no copy ctor...
  for (auto iter = aProviderDict->ConstIter(); !iter.Done(); iter.Next()) {
    mProviderDict.Put(iter.Key(), new nsCString(*(iter.Data())));
  }
}
...
Classifier::GetProvider(const nsACString& aTableName)
{
  ...
  return provider ? *provider : EmptyCString();
}
