                                          ...
                                          const nsACString & aStreamTable)
{
...
#ifdef DEBUG
  {
    ...
    LOG(("Fetching update %s from %s", aRequest.Data(), spec.get()));
  }
#endif
  ...
  if (!aIsPostRequest) {
    ...
    rv = httpChannel->SetRequestHeader(NS_LITERAL_CSTRING("X-HTTP-Method-Override"),
                                       ...
                                       false);
    NS_ENSURE_SUCCESS(rv, rv);
  } else if (!aRequest.IsEmpty()) {
    rv = AddRequestBody(aRequest);
    NS_ENSURE_SUCCESS(rv, rv);
  }
                                          ...
                                          const nsACString & aStreamTable)
{
  ...
  if (!aIsPostRequest) {
    updateUrl.AppendPrintf("&$req=%s", nsCString(aRequest).get());
  }
  ...
  return FetchUpdate(uri, aRequest, aIsPostRequest, aStreamTable);
}
  ...
  bool *_retval)
{
  ...
  if (mIsUpdating) {
    LOG(("Already updating, queueing update %s from %s", aRequest.Data(),
         aUpdateUrl.Data()));
    *_retval = false;
    PendingRequest *request = mPendingRequests.AppendElement();
    request->mTables = aRequestTables;
    request->mRequest = aRequest;
  ...
  *_retval = true;
  ...
  return FetchUpdate(aUpdateUrl, aRequest, aIsPostRequest, EmptyCString());
}
